// 学習アプリ用のDB
const db = new Dexie("PracticeAppDB_GoogleCloud_PDE");
// 学習進捗用のデータストア
db.version(1).stores({
...
const createContentTree = () => {
  ...
  $("h2, h3").each((_, e) => {
    ...
    if ("H2" === $(e).prop("tagName")) {
      ...
      details = $(
        `<details data-for-header='${id}'><summary><a href="#${id}">${$(e).children("span").text()}</a></summary></details>`
      );
      ul = $("<ul></ul>");
...
const initPracticeProgress = async () => {
  ...
  if (option) {
    ...
    $.makeArray($("input"))
      .filter((e) => $(e).attr("id").startsWith("toggle-complete-") && $(e).prop("checked"))
